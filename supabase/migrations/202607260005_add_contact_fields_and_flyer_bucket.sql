...
drop policy if exists "eventos flyers lectura publica" on storage.objects;

create policy "eventos flyers lectura publica"
on storage.objects
for select
to anon, authenticated
using (bucket_id = 'eventos-flyers');

drop policy if exists "eventos flyers carga autenticada" on storage.objects;

create policy "eventos flyers carga autenticada"
on storage.objects
for insert
to authenticated
with check (bucket_id = 'eventos-flyers');

drop policy if exists "eventos flyers actualizacion autenticada" on storage.objects;

create policy "eventos flyers actualizacion autenticada"
on storage.objects
for update
to authenticated
using (bucket_id = 'eventos-flyers')
...
drop policy if exists "eventos flyers borrado autenticado" on storage.objects;

create policy "eventos flyers borrado autenticado"
on storage.objects
for delete
to authenticated
using (bucket_id = 'eventos-flyers');
...
begin
  ...
  if v_telefono is not null
    and (
      length(v_telefono) > 30
      or length(regexp_replace(v_telefono, '\D', '', 'g')) < 6
    ) then
